...
For anything beyond what `craft.toml` currently supports, add a `CMakeLists.before.cmake` or a `CMakeLists.after.cmake` file to your project root. Craft will include it in the generated `CMakeLists.txt` automatically. 

These extra cmake files should be committed to your repository. `CMakeLists.txt` should be gitignored since it is generated by Craft.
...
A Craft workspace allows multiple related Craft projects to be built together from a single root directory. Dependencies are shared across members and built once, making workspace builds significantly faster than building each project independently.

---

## Overview

A workspace is defined by a `craft.toml` at the root containing a `[workspace]` section:
...
Each member is the path to an independent Craft project with its own `craft.toml`. Members can depend on each other using regular path dependencies. Craft handles build ordering automatically.

---

## Setting Up a Workspace

Run `craft workspace` in the root directory of your multi-project setup:
...
Craft will:
1. Scan the directory recursively for Craft projects
2. Generate a `craft.toml` with a `[workspace]` section listing all detected members
3. Generate a root `CMakeLists.txt` that builds all members together
4. Fetch all member git dependencies into the workspace `.craft/deps/`

### Typical Workspace Structure

```
physics_engine/
├── craft.toml              # [workspace] only
├── CMakeLists.txt          # generated by Craft
├── .craft/
│   └── deps/               # shared dependencies for all members
├── engine/
│   ├── craft.toml
│   └── src/
└── demos/
...
Run `craft build` from the workspace root or any subdirectory that is not a project:
...
Craft detects the workspace root automatically by walking up the directory tree. All members are built in dependency order. If `demo1` depends on `engine`, Craft builds `engine` first.

Note: `craft clean` can also be used in a workspace.

---

## Shared Dependencies

Git dependencies are shared across the entire workspace. If multiple members depend on the same library it is cloned once into the workspace `.craft/deps/` and built once. Members do not maintain their own `.craft/deps/` when built as part of a workspace.

```
workspace/.craft/deps/
├── raylib/          # shared by all members that need it
└── SFML/            # shared by all members that need it
```

---

## Standalone Member Builds

Each member can still be built independently outside the workspace context. When built standalone, a member fetches its own dependencies into its local `.craft/deps/`:

```bash
cd physics_engine/demos/demo1
craft build    # builds standalone, fetches its own deps
```

This is useful for working on a single member without building the entire workspace.

---

## Member Dependencies

Members reference each other using regular path dependencies in their `craft.toml`:

```toml
# demos/demo1/craft.toml
[dependencies]
engine = { path = "../../engine" }
raylib = { git = "https://github.com/raysan5/raylib.git", tag = "5.5", links = ["raylib"]}
```

Craft uses these dependency relationships to determine build order within the workspace.