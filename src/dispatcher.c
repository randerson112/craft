...
int dispatch(command_t* command_data) {
    ...
    if (strcmp(command, "project") == 0)  return project(command_data);
    if (strcmp(command, "init") == 0)     return init(command_data);
    if (strcmp(command, "build") == 0)    return build();
    if (strcmp(command, "compile") == 0)  return compile(command_data);
    if (strcmp(command, "run") == 0)      return run(command_data);
    if (strcmp(command, "gen") == 0)      return gen(command_data);
    if (strcmp(command, "clean") == 0)    return clean();
    if (strcmp(command, "help") == 0)     return help(command_data);
    ...
    if (strcmp(command, "add") == 0)      return add(command_data);
